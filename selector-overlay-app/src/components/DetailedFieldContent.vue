<template>
  <div class="space-y-5">
    ...
    <div>
      <label class="block text-base font-medium text-gray-700 mb-2">
        ...
        <button
          ...
          class="ml-2 text-sm text-blue-600 hover:text-blue-700 underline font-normal"
        >
          Test
        </button>
      </label>
    ...
    <div class="border-t pt-4">
      ...
      <div class="space-y-2">
        <label class="block text-sm text-gray-600 mb-2">Quick Transformations</label>
      ...
      <div class="mt-3">
        <label class="block text-sm text-gray-600 mb-2">More Transformations</label>
        <div class="flex flex-wrap gap-2">
          <button
      ...
      <div class="mt-3">
        <label class="block text-sm text-gray-600 mb-2">Custom Regex (Advanced)</label>
        <div class="space-y-2">
          <input
      ...
      <div class="mt-4 border-t pt-3">
        ...
        <div v-if="showJsEditor" class="space-y-2">
          <div class="text-sm text-gray-600 bg-yellow-50 border border-yellow-200 rounded p-3 mb-2">
            <strong>💡 Tip:</strong> Use <code class="bg-yellow-100 px-1.5 py-0.5 rounded text-xs">value</code> variable to access extracted data. 
            Return the transformed result. Example: <code class="bg-yellow-100 px-1.5 py-0.5 rounded text-xs">return value.split(',')[0]</code>
          </div>
          ...
          <textarea
            ...
            class="w-full px-4 py-3 border border-gray-300 rounded font-mono text-sm focus:ring-2 focus:ring-blue-500 bg-gray-900 text-green-400"
            spellcheck="false"
          />
          
          <div v-if="jsCodeError" class="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">
            ⚠️ <strong>Error:</strong> {{ jsCodeError }}
          </div>
          
          <div v-if="customJsCode && !jsCodeError" class="text-sm text-green-600 bg-green-50 border border-green-200 rounded p-3">
          ...
          <div class="flex gap-2">
            <button
              @click="applyJsCode"
              class="flex-1 px-4 py-2.5 text-sm bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded hover:from-purple-600 hover:to-indigo-700 transition-colors font-medium"
            >
              💾 Apply JS Code
            </button>
            <button
              @click="clearJsCode"
              class="px-4 py-2.5 text-sm bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition-colors"
            >
              Clear
            </button>
          </div>
      ...
      <div v-if="activeTransformations.length > 0" class="mt-3">
        <label class="block text-sm text-gray-600 mb-2">Active Transformations</label>
        <div class="space-y-2">
          <div
            v-for="(trans, idx) in activeTransformations"
            :key="idx"
            class="flex items-center justify-between bg-blue-50 px-3 py-2 rounded text-sm"
          >
    ...
    <div class="border-t pt-4">
      <div class="flex items-center justify-between mb-2">
        ...
        <button
          ...
          class="text-sm px-3 py-1.5 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors font-medium"
        >
      ...
      <div v-if="livePreviewOriginal" class="space-y-2">
        ...
        <div v-if="comparisonView === 'both' && activeTransformations.length > 0" class="grid grid-cols-2 gap-2">
          ...
          <div class="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-300 p-3 rounded-lg">
            <div class="text-sm text-gray-600 mb-1 font-semibold flex items-center">
              📝 Original
            </div>
          ...
          <div class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-400 p-3 rounded-lg">
            <div class="text-sm text-green-700 mb-1 font-semibold flex items-center">
              ✨ Transformed
            </div>
        ...
        <div v-else class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-300 p-4 rounded-lg">
        ...
        <div v-if="activeTransformations.length > 0" class="text-sm text-gray-600 bg-blue-50 border border-blue-200 rounded p-3">
      ...
      <div v-else class="bg-gray-50 p-4 rounded-lg text-base text-gray-500 italic">
    ...
    <div class="border-t pt-4">
      <div class="flex items-center justify-between mb-3">
        <label class="block text-base font-semibold text-gray-700">
          🔍 Test Results
          <span v-if="props.testResults.length > 0" class="ml-2 px-2.5 py-1 text-sm bg-green-100 text-green-800 rounded-full font-semibold">
            {{ props.testResults.length }} {{ props.testResults.length === 1 ? 'match' : 'matches' }}
          </span>
        </label>
        <button
          @click="testCurrentSelector"
          class="px-5 py-2.5 text-base bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors font-medium shadow-sm"
        >
          Run Test
        </button>
      </div>
        ...
        <div v-if="props.testResults.length > 0" class="mt-3">
          <!-- Results Header -->
          <div class="bg-gradient-to-b from-blue-50 to-blue-100 px-4 py-3 border-2 border-blue-200 rounded-t-lg">
